...
log_path = os.path.join(get_project_path(), "output_analysis", "HF_analysis", "graphs", "processing_log.txt")
output_path = os.path.join(get_project_path(), "output_analysis", "HF_analysis", "graphs", "HF_coordinates.txt")
...
try:
    ...
    if len(aexx_values) != len(energies) or len(aexx_values) != len(band_gaps):
        raise ValueError("Mismatch in the number of AEXX, Energy, and Band Gap values.")
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig(os.path.join(get_project_path(), "output_analysis", "HF_analysis", "graphs", "Energy_vs_AEXX.png"),
                dpi=1200)
    plt.show()
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig(os.path.join(get_project_path(), "output_analysis", "HF_analysis", "graphs", "BandGap_vs_AEXX.png"),
                dpi=1200)
    plt.show()
